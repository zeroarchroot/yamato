...
(function() {
  ...
  // Inject global styles to hide media early
  ...
  function processNode(node) {
    ...
    // Block background images in styles
    if (node.tagName === 'STYLE' || node.tagName === 'LINK') {
      // For dynamic styles, but hard to block; rely on global CSS
    }
  ...
  pointerEvents.forEach(ev => window.addEventListener(ev, blockEvent, {capture: true, passive: false}));
  keyEvents.forEach(ev => window.addEventListener(ev, blockKeyEvent, {capture: true, passive: false}));
  ...
  // Additional observer for style changes if possible, but MutationObserver doesn't catch CSS changes easily
  // For now, re-process on DOMContentLoaded
  ...
  (browser.runtime.sendMessage || chrome.runtime.sendMessage)({action: 'play-sound'});
  ...
  window.addEventListener('unload', () => {
    pointerEvents.forEach(ev => window.removeEventListener(ev, blockEvent, {capture: true}));
    keyEvents.forEach(ev => window.removeEventListener(ev, blockKeyEvent, {capture: true}));
    mo.disconnect();
  }, {once: true});
})();
